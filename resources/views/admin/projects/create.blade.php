...
<div class="mb-6">
    <h2 style="font-size: 1.25rem; font-weight: 700;">{{ isset($project) ? 'Edit Project' : 'Create Project' }}</h2>
</div>
        ...
        <div class="col-span-2">
            <div class="card mb-6">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-input" name="name" value="{{ old('name', $project->name ?? '') }}" required>
        ...
        <div>
            <div class="card mb-6">
                ...
                <div class="form-group">
                    ...
                    <input type="number" class="form-input" name="sort_order" value="{{ old('sort_order', $project->sort_order ?? 0) }}">
                </div>
            </div>
            ...
            <div class="card mb-6">
                ...
                <div class="form-group">
                    ...
                    @if(isset($project) && $project->thumbnail)
                        <img src="{{ asset('storage/' . $project->thumbnail) }}" style="margin-top: 0.5rem; border-radius: var(--radius); max-height: 150px;" alt="Current thumbnail">
                    @endif
                </div>
            </div>
                ...
                <div class="form-group">
                    @foreach($tags as $tag)
                        <label style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.25rem; font-size: 0.9375rem;">
                            <input type="checkbox" name="tags[]" value="{{ $tag->id }}" {{ in_array($tag->id, old('tags', isset($project) ? $project->tags->pluck('id')->toArray() : [])) ? 'checked' : '' }}>
                            {{ $tag->name }}
                        </label>
                    @endforeach
                </div>
            ...
            <div class="flex gap-2">
                <button type="submit" class="btn btn-primary w-full">{{ isset($project) ? 'Update' : 'Create' }} Project</button>
